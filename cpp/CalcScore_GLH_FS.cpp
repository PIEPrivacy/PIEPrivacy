...
int main(int argc, char *argv[])
{
	...
	sprintf(etrace_file, "%s/testtraces_%s_%s.csv", outdir, city, obf);
...
//	for (i = 0; i < muser_num; i++) printf("%d,%d\n", i, muser[i]);
	...
	*/
	...
	sprintf(outfile, "%s/ge_score_%s_%s_ak%d_rm%d_rnl%d_nl%d.csv", outdir, city, obf, adv_know, reid_method, req_num_loc, num_loc);
	...
	sprintf(outfile, "%s/im_score_%s_%s_ak%d_rm%d_rnl%d_nl%d_r%d.csv", outdir, city, obf, adv_know, reid_method, req_num_loc, num_loc, rnd_imscore);
