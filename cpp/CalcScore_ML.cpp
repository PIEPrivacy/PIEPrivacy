...
void ReadTTraceFile(char *ttrace_file, map<int, int> &meet_req, map<int, double> *visit_vec, map<tuple<int, int>, double> *trans_mat, int num_loc) {
...
int main(int argc, char *argv[])
{
	...
	if (argc < 4) {
		...
		printf("[City]: City (AL)\n");
		printf("[Obf]: Obfuscation (org/RR-e[Epsilon]-l[TraceLen])\n");
...
//	for (i = 0; i < muser_num; i++) printf("%d,%d\n", i, muser[i]);
	...
	ReadTTraceFile(ttrace_file, meet_req, visit_vec, trans_mat, num_loc);
	/*
