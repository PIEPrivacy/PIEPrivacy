...
// Read POIs
void ReadPOIs(char *poi_file, map<int, int> &poi_exist){
	int poi_index;
	char s[1025];
	FILE *fp;
	char *tok;
	...
	// Open the POI file
	fp = FileOpen(poi_file, "r");
	// Skip the header
	fgets(s, 1024, fp);
	// Read POIs from the POI file --> poi_exist
...
{
	char outdir[129], city[129], ttrace_file[129], etrace_file[129], obftrace_file[129], poi_file[129], per_tra[129];
	char eps_s[129];
	double eps;
	int trace_len;
	...
	if (argc < 3) {
		printf("Usage: %s [DataDir] [City] ([Epsilon (default:1)] [TraceLen (default:1)] [PerTra (default:4.0)])\n\n", argv[0]);
		...
		printf("[PerTra]: Percentage of Training Data\n\n");
		return -1;
	}
	...
	strcpy(per_tra, "4.0");
	if (argc >= 6) strcpy(per_tra, argv[5]);
//	printf("%s,%s,%s,%f,%d,%s\n", outdir, city, eps_s, eps, trace_len, per_tra);

//	sprintf(ttrace_file, "%s/traintraces_%s.csv", outdir, city);
	sprintf(ttrace_file, "%s/testtraces_%s.csv", outdir, city);
	...
	sprintf(poi_file, "%s/movieindex_%s.csv", outdir, per_tra);
